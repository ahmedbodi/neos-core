...
#if defined(NDEBUG)
#error "NEOS cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "NEOS Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight+1 == 2) {
      nSubsidy = 3873344 * COIN; // Money supply premine for the swap
    } else if (nHeight+1 == 3) {
      nSubsidy = 100000 * COIN; // NEOS for staking wallets and masternodes to start the new chain (8 masternodes = 40k NEOS, 3 staking wallets with 20k NEOS each)
    } else if (nHeight+1 >= 3 && nHeight+1 < 500000) {
      nSubsidy = 0.80 * COIN;
    } else if (nHeight+1 >= 500000 && nHeight+1 < 1000000) {
      nSubsidy = 0.70 * COIN;
    } else if (nHeight+1 >= 1000000 && nHeight+1 < 1500000) {
      nSubsidy = 0.60 * COIN;
    } else if (nHeight+1 >= 15000000) {
      nSubsidy = 0.50 * COIN;
    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    // Check timestamp
    if (block.GetBlockTime() < GetAdjustedTime() + (block.IsProofOfStake() ? 60 : 7200)) // 1 minute future drift for PoS
        return state.Invalid(error("CheckBlock() : block timestamp too far in the past"),
